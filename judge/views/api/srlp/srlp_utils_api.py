...
def filter_if_not_none(qs, **kwargs):
    return qs.filter(**{k: v for k, v in kwargs.items() if v is not None and v != ''})
...
def order_by_if_not_none(qs, list_params):
    return qs.order_by(*[v for v in list_params if v is not None and v != ''])
...
class isVisitante(permissions.BasePermission):
    def has_permission(self, request, view):
        if(bool(request.user and request.user.is_authenticated)):
            return bool(request.user.profile.display_rank == "Visitante")
        else: return False
